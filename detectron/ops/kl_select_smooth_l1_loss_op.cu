...
template<>
bool kl_selectSmoothL1LossOp<float, CUDAContext>::RunOnDevice() {
...
template<>
bool kl_selectSmoothL1LossGradientOp<float, CUDAContext>::RunOnDevice() {
  auto& Y_hat      = Input(0);
  auto& alpha      = Input(1);
  auto& Y          = Input(2);
  auto& L          = Input(3);
...
REGISTER_CUDA_OPERATOR(kl_selectSmoothL1Loss,
                       kl_selectSmoothL1LossOp<float, CUDAContext>);
REGISTER_CUDA_OPERATOR(kl_selectSmoothL1LossGradient,
                       kl_selectSmoothL1LossGradientOp<float, CUDAContext>);
}  // namespace caffe2
