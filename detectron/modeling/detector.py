...
from detectron.ops.cov_loss import *
...
class DetectionModelHelper(cnn.CNNModelHelper):
    ...
    def CovLoss(self, blobs_in, blobs_out):
        name = 'CovLoss' + ','.join([str(b) for b in blobs_in])
        self.net.Python(CovLossOp().forward, CovLossOp().backward, grad_input_indices=[0, 1])(blobs_in, blobs_out, name=name)
        return blobs_out

    def LogDet(self, blobs_in, blobs_out):
        name = 'LogDet' + ','.join([str(b) for b in blobs_in])
        self.net.Python(LogDetOp().forward, LogDetOp().backward)(blobs_in, blobs_out, name=name)
        return blobs_out

    def UpperTriangular(self, blobs_in, blobs_out):
        name = 'UpperTriangular' + ','.join([str(b) for b in blobs_in])
        self.net.Python(UpperTriangularOp().forward, UpperTriangularOp().backward)(blobs_in, blobs_out, name=name)
        return blobs_out

    def SigmaL1(self, blobs_in, blobs_out):
        name = 'SigmaL1' + ','.join([str(b) for b in blobs_in])
        self.net.Python(SigmaL1Op().forward, SigmaL1Op().backward)(blobs_in, blobs_out, name=name)
        return blobs_out

    def Lock(self, blobs_in, blobs_out):
        name = 'Lock' + ','.join([str(b) for b in blobs_in])
        self.net.Python(LockOp().forward, LockOp().backward)(blobs_in, blobs_out, name=name)
        return blobs_out

    def LogSumExp(self, blobs_in, blobs_out):
        name = 'LogSumExp' + ','.join([str(b) for b in blobs_in])
        self.net.Python(LogSumExpOp().forward, LogSumExpOp().backward)(blobs_in, blobs_out, name=name)
        return blobs_out
